...
import GhostBorders from "@/components/atlas/GhostBorders";
...
function MapFlyTo({ target }) {
  ...
  useEffect(() => {
    if (target && typeof target.lat === "number") {
      ...
      map.flyTo([target.lat, target.lng], target.zoom || 8, { duration: reduce ? 0 : 1.2 });
    }
  }, [target]);
  return null;
}

export default function WorldMap({ activePeriod, darkMode, mode, onCountrySelect, onCitySelect, highlightedCountryNames, blinkingCity, flyTarget, comparePeriod }) {
  const geoJsonUrl = getGeoJsonUrl(activePeriod);
  const compareUrl = mode !== "epistemological" && comparePeriod ? getGeoJsonUrl(comparePeriod) : null;
  ...
  return (
    ...
    >
      ...
      {compareUrl && <GhostBorders geoJsonUrl={compareUrl} />}
